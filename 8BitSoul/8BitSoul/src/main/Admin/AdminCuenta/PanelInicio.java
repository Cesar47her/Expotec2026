...
public class PanelInicio extends JPanel {
    ...
    public void actualizarDatosPantalla() {
        ...
        String totalUsuarios = InicioDAO.obtenerTotalUsuarios();
        String totalCompras = InicioDAO.obtenerTotalCompras();
        String totalTickets = InicioDAO.obtenerTotalTickets();
        String totalInventario = InicioDAO.obtenerTotalInventario();
        String totalVentas = InicioDAO.obtenerTotalVentas();

        grid.add(crearCardMetrica("👥", totalUsuarios, "USUARIOS REGISTRADOS", "+1.2%", true));
        grid.add(crearCardMetrica("🛒", totalCompras, "COMPRAS REALIZADAS", "+4.5%", true));
        grid.add(crearCardMetrica("🎧", totalTickets, "SOLICITUDES DE AYUDA", "Estable", true));
        grid.add(crearCardMetrica("📦", totalInventario, "OBJETOS EN INVENTARIO", "Sincronizado", true));
        grid.add(crearCardMetrica("💲", totalVentas, "VENTAS TOTALES", "Live", true));

        panelContenedorEstadisticas.add(grid, BorderLayout.CENTER);
